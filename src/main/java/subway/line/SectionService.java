...
public class SectionService {
    ...
    public ResponseEntity create(Long id, SectionRequest sectionRequest) {
    ...
    public void validate(Long id, SectionRequest sectionRequest, List<Station> stations) {
        if (hasDuplicatedStation(id, sectionRequest, stations)) {
            throw new IllegalArgumentException("상행역과 하행역이 이미 노선에 모두 등록되어 있다면 추가할 수 없음");
        }

        if (!containsEndStation(id, sectionRequest, stations)) {
    ...
    public ResponseEntity delete(Long id, Long stationId) {
